...
df <- dplyr::bind_rows(list(df16, df17,df18_1,df18_2,df19_1,df19_2,df20,df21))
  ...
  group_by(Year) %>% summarize("Running days" = n()
                               ...
                               , "....Snowy" =
                                 paste0(sum(weather=="Snowy"),
                                        " (",
                                        round(sum(weather=="Snowy")/`Running days`,3) * 100,
                                        ")")
                               , "....Cloudy" =
...
running_summary <- t(summary_table) %>% 
  kableExtra::kbl(caption = paste0("Running Sammry: ", min(df$Year)," - ",max(df$Year), " (updated on ",Sys.Date(),")")) %>%
...
```{r daily_minutes}
df$fills <- df$`runningTime(min)`
gh_waffle(df[df$Year=='2021', ], 'Time (minutes)')
```
## 2021
```{r daily_miles}
df$fills <- df$`Distance(mile)`
gh_waffle(df[df$Year=='2021', ], 'Miles')
table(df$Year,df$Shoes)
```